...
using me.cqp.luohuaming.AbyssUploader.Core;
using me.cqp.luohuaming.AbyssUploader.Core.Domain;
using me.cqp.luohuaming.AbyssUploader.Sdk.Cqp;
using me.cqp.luohuaming.AbyssUploader.Sdk.Cqp.Enum;
using me.cqp.luohuaming.AbyssUploader.Sdk.Cqp.EventArgs;
using me.cqp.luohuaming.AbyssUploader.Sdk.Cqp.Interface;
using me.cqp.luohuaming.AbyssUploader.Sdk.Cqp.Expand;
using me.cqp.luohuaming.AbyssUploader.Sdk.Cqp.Model;
using Unity;
using Unity.Injection;

namespace me.cqp.luohuaming.AbyssUploader.App.Export
{
	/// <summary>	
	/// 表示酷Q事件导出的类	
	/// </summary>	
	...
	{	
		...
		{	
			return "9,me.cqp.luohuaming.AbyssUploader";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("me.cqp.luohuaming.AbyssUploader", 1, 9, "深渊慢报", "1.0.0", 1, "落花茗", "群策群力", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("me.cqp.luohuaming.AbyssUploader", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("me.cqp.luohuaming.AbyssUploader", AppData.CQLog);	
